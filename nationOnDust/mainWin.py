import sys, os, random, time, math, shutil, traceback
...
allTexts = [
    ["controls:", "wasd/qe/enter", "1-notes 2-population", "3-influence 4-groups", "5-builds", "menu:", "new",
     "commands:"] +
    commands[:5] + ["aliases:"] + aliases + ["!advance help!:", "investment-", "# [Y/n] to enable", "eg- '3 y'",
                                             "[citizens,influence,", "military,time]", "build-",
                                             "(maintained-_ mound-A farm-#)"],  # state0
    [],
...
codeInv = [[0.1, 0.2, 0.0, 10], [0.2, 0.1, 0.1, 15], [0.1, 0.1, 0.0, 9], [0.1, 0.0, 0.2, 4], [0.2, 0.2, 0.3, 10],
           [0.3, 0.1, 0.1, 20], [0.3, 0.0, 0.0, 15], [0.1, 0.1, 0.1, 6], [0.1, 0.1, 0.1, 6]]  # 0treaty envoy healer 3ceremony raid union 6mound migrateWest 8migrateEast
reset = "\033[0m"
...
def disableControls():
    # Windows doesn't need terminal mode changes like Unix
    pass
...
def printLine(y, text=""):
    # Use Windows-compatible cursor positioning
    ...
    for y in range(v.mapSize[1]):
        for x in range(v.mapSize[0]):
            ...
            if n > v.threshold / 100:
                n2 = (max(0.05, noiseTwo.noise2(x * v.scale / v.mapSize[0] * 1.8,
                                                 y * v.scale / v.mapSize[1] * 1.8)) * 110)
                dx = x - v.mapSize[0] / 2
                dy = y - v.mapSize[1] / 2
...
def gameLoop():
    ...
    if v.delay % max(1, int(80 * v.speed)) == 0:
        ...
        for i in range(len(v.players[v.pId].influences)):
            ...
            v.cells[oldInf[1]].population = max(0, min(9999999, v.cells[oldInf[1]].population - totalMil) + oldInf[
                0] / val1 * v.cells[oldInf[1]].population / 22 * hasFarm)
            v.cells[oldInf[1]].population = max(0, v.cells[oldInf[1]].population / (v.players[v.pId].disease[0] * 0.05 + 1))
...
def consoleShow():
    ...
    finalText, added = [], ["", ""]
    if v.windowState == 2:
        added = ["", v.players[v.pId].citizens, v.players[v.pId].military,
                 ["none", "mild", "heavy"][v.players[v.pId].disease[0]]]

    elif v.windowState == 3:
        focusStr = " ".join(
            [f"!{word}!" if i == v.players[v.pId].focus else word for i, word in enumerate(commands[5:8])])
        added = ["", focusStr]
        ...
        total = (peakIdx[0] / 2 + peakIdx[1] * peakIdx[2]) / 3 + (
                    v.players[v.pId].citizens / 2 + v.players[v.pId].military * allInf)
